...
import type { ChatRequest, LegacyLocalStorageImport, CenterSettings } from "@ai-control-center/contracts"
import { CenterSettingsSchema } from "@ai-control-center/contracts"
...
export async function registerApiRoutes(app: FastifyInstance, services: CenterServices): Promise<void> {
  ...
  app.get("/services", async () => services.serviceCatalog.list())

  app.put<{ Body: CenterSettings }>("/settings", { schema: { body: CenterSettingsSchema } }, async (request) => {
